...
export interface TabProps {
  defaultIndex?: number;
  styleType?: TabStyle;
  onSelect?: (selectdIndex: number) => void;
  className?: string;
}
...
export const Tabs: FC<TabProps> = (props) => {
  ...
  const childrenComponent = () => {
    return React.Children.map(children, (child, index) => {
      ...
      const isLableDisabled = childElement.props.disabled
      ...
      const handleChildClick = () => {
        handleClick(index, isLableDisabled);
      };
      return (
  ...
  const renderChildren = () => {
    return React.Children.map(children, (child, index) => {
      ...
      if (displayName === "TabsItem") {
        ...
        console.error(
          "Warning: Tabs has a child which is not a TabsItem component"
        );
      }
    });
  };
